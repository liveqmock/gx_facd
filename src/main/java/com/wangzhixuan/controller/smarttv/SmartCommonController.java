...
public class SmartCommonController extends BaseController {
	...
	public Object mapmark(HttpServletRequest req, String token) {
		...
		try {
			...
			for (MapMark mm : mms) {
				...
				json.put("name", mm.getName());
				json.put("type", mm.getType());
				json.put("longitude", mm.getLongitude());
				json.put("latitude", mm.getLatitude());
				json.put("info", mm.getInfo());
				json.put("createTime", new SimpleDateFormat("yyyy-MM-dd HH:mm").format(mm.getCreateTime()) );
				json.put("icon", url+mm.getIcon());
				jsonArray.add(json);
			}
	...
	public Object onlineLive(String token, HttpServletRequest request) {
		...
		try {
			...
			lv.setReceiveId(user.getUuid());
			lv.setCreateTime(new Date());
			lv.setType(2);
			lv.setPatrolid(request.getParameter("patrolid"));
			lv.setLiveId(request.getParameter("receiveId"));
			lv.setUrl(url);
			lv.setStatus(0);
