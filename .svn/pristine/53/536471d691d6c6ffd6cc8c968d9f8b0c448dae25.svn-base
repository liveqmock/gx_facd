...
public class RoadController extends BaseController {
	...
	public Object add(@Valid Road road, BindingResult result) {
		...
		road.setUuid(UUID.randomUUID().toString());
	...
	public Object delete(String uuid) {
		...
		EntityWrapper<Road> ew1 = new EntityWrapper<>();
		ew1.eq("uuid", uuid);
		Road road = new Road();
		road.setDataStatus(2);
		boolean b = roadService.update(road, ew1);
		if (b) {
	...
	public String editPage(Model model, String uuid) {
		Road road = roadService.selectByUUid(uuid);
